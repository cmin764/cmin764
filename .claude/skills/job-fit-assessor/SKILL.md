...
description: >
  ...
  role against their experience. The output is a filterable, collapsible React artifact
  with an overall fit score and optional PDF export.
---
...
### 1.1 Local files (when running inside a repo)

Scan the working directory for profile-related files. Common patterns:
...
If the repo contains a profile README (e.g. `username/username` on GitHub), that's
the primary source.
