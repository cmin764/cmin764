...
### In Claude Code (inside a repo with profile files)

Claude scans the local directory for profile files automatically. You just provide
the JD:
...
```
Assess my fit against this JD: [paste]
```

You can also point Claude at additional materials:
...
Claude handles the rest: reading the profile, researching the company, mapping
requirements, producing the artifact.
